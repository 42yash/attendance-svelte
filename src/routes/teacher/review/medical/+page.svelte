<script>
	...
	async function getClaimsList() {
		if (browser) {
			...
			const response = await fetch(`${env.GO_API_KEY}/teacher/claims`, {
				headers: {
			...
			if (response.ok) {
				...
				rawData.forEach((claim) => {
					entries.push({
						id: claim.MedicalClaim.ID,
						date: claim.CreatedAt.slice(0, 10),
						reason: claim.MedicalClaim.Reason,
						program: claim.MedicalClaim.Student.Class
					});
				});
				return;
			} else {
